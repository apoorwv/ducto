...
ducto is a drop-in credit calculation engine. Define pricing as math expressions (per-model, per-tool, search/RAG, cache, fixed jobs), connect a database, and start deducting credits. For streaming and agentic workloads, the lease pattern (`reserve → settle / release`) lets you hold credits during execution and bill the actual cost at the end — with atomic admission control and no double-charges.
...
Creates `user_credits`, `credit_transactions`, `credit_reservations` tables (and the pricing, plan, team, cap and analytics objects) — idempotent, safe to run on every deploy. The connection string comes from `DATABASE_URL`, not the command line.
...
- [Financial Safety (Leases)](/docs/financial-safety) — atomic reserve → settle lifecycle, presets, and zero-debt guarantees
