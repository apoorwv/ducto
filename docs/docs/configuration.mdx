...
## Version 2 (v2) — Plan-Based Pricing

Extends v1 with subscription plan definitions. Plans give users free monthly allowances, rate overrides, and feature gating metadata.
...
# From dict (v1 or v2)
engine = PricingEngine.from_dict({"version": 1, "models": {"_default": "input_tokens * 1"}})
...
const engine = PricingEngine.fromDict({ version: 1, models: { _default: "input_tokens * 1" } });
...
1. Version field (`1` or `2`)
2. `models` is non-empty
3. All expression strings parse and are safe
4. Only allowed functions used
5. Plan names are unique (v2)
6. Plan `rate_overrides` expressions are valid (v2)
7. No unknown sections
