...
### `reserveCredits(userId: string, amount: Decimal | number, operationType?: string, metadata?: CreditMetadata | null, minBalance?: Decimal | number | null): Promise<ReserveResult>`

Legacy hold via `credit_reservations`. Prefer the lease lifecycle (`reserve` / `settle` / `release`) for new code. Returns `{ reservationId, userId, amount, balance, reservedTotal, error? }`.
